...
def category_spent_sum(user,category,date_start,date_end):
    if category == None:
        category_spent_total = trans.objects.aggregate(total=Sum('amount', filter=Q(user_id=user, date__range=(date_start, date_end))))['total']    
    else:
        category_spent_total = trans.objects.aggregate(total=Sum('amount', filter=Q(user_id=user, category_id=category, date__range=(date_start, date_end))))['total']    
...
def budget_status(user,category,date_start,date_end, category_target_total,month_id,year_id, days_month):
    print(user,category,date_start,date_end, category_target_total,month_id,year_id, days_month)
    if category == None:    
        spent_total = trans.objects.aggregate(total=Sum('amount', filter=Q(user_id=user, category_id__Fixed_fees=False ,date__range=(date_start, date_end))))['total']
        ...
        spent_total = trans.objects.aggregate(total=Sum('amount', filter=Q(user_id=user, category_id=category, category_id__Fixed_fees=False ,date__range=(date_start, date_end))))['total']
